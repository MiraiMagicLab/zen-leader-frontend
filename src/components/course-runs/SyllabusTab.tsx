...
const typeStyleMap: Record<string, string> = {
  ...
  photo: "text-primary bg-primary/5",
      ...
      <Card className={cn("overflow-hidden border-border shadow-sm", isDragging && "ring-2 ring-primary")}>
        <div className="flex items-center justify-between p-4 bg-card hover:bg-muted/50 transition-colors border-b border-border/50">
          ...
          <div className="flex items-center gap-2 ml-4">
            <Button variant="ghost" size="sm" onClick={onAddLesson} className="h-8 gap-1.5 text-primary font-bold hover:bg-primary/10">
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              <CardContent className="p-4 space-y-2 bg-muted/20">
                {children}
                {(!chapter.lessons || chapter.lessons.length === 0) && (
                  <div className="rounded-xl border border-border/50 bg-background/50 py-8 text-center text-muted-foreground/50">
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground/70">{lesson.type}</span>
            ...
            {Boolean(lesson.contentData?.fileAttachment) && <Paperclip className="size-3 text-primary/70" />}
          </div>
        </div>
      </div>
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={onEdit}>
...
export function SyllabusTab({ runId }: SyllabusTabProps) {
  ...
  if (queryChapters.isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3 text-muted-foreground/30">
    ...
    <div className="space-y-6">
      ...
      <Sheet open={lessonSheetOpen} onOpenChange={setLessonSheetOpen}>
        <SheetContent side="right" className="w-full sm:max-w-[760px] flex flex-col overflow-hidden border-border p-0 rounded-l-xl">
          <SheetHeader className="relative border-b border-border/10 bg-muted/20 p-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                       <select 
                        ...
                        className="h-10 w-full cursor-pointer appearance-none rounded-xl border border-border bg-muted/50 px-4 pr-10 text-sm font-medium hover:bg-background focus:ring-2 focus:ring-primary/20 outline-none"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div 
                      className={cn(
                        ...
                        lessonForm.isHidden ? "bg-destructive/5 border-destructive/20" : "bg-card border-border hover:border-muted-foreground/30"
                      )}
                    ...
                    <div 
                      className={cn(
                        "cursor-pointer select-none rounded-xl border p-5 transition-colors",
                        lessonForm.isOptional ? "bg-primary/5 border-primary/20" : "bg-card border-border hover:border-muted-foreground/30"
                      )}
          ...
          <div className="flex items-center justify-between border-t border-border/10 bg-muted/30 p-6">
